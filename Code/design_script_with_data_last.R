...
if(length(args) != 8){
  stop("Please pass valid values for theta0, theta1, beta, C (T or F), (D or T or S), Y, rep number and N")
}


#initial_design = c(5 ,9, 6, 9 ,1, 5, 1, 8, 4, 6, 5, 5, 2, 10, 5, 3, 7, 2, 5, 4)

candidate = 1:10
#candidate = c(1 ,2.5, 4, 5.5, 7, 8.5,10 )
#n <- length(initial_design)  # Number of values in each simulated dataset

#N = 30
# If we want censoring, what is the censoring time Y (censoring time = end of study)
#Y = 2

#data_path = "C:/Users/han-s/Downloads/SimDataPH/"
theta0 = as.numeric(args[1])
theta1 = as.numeric(args[2])
input_beta = as.numeric(args[3])
# Here we decide if we wish to apply censoring
censor = as.logical(args[4])
opt.type = args[5]
method = ""
if(opt.type == "D"){
  method = "det"
...
# change path on the cluster
#data_path = paste("C:/Users/han-s/Downloads/data_",as.character(theta0),"_",as.character(theta1),"_",as.character(input_beta),"_",as.character(censor),"_",method,"_",as.character(rep_num),"/", sep = "")
data_path = paste("/scratch/users/k1814473/data_",as.character(theta0),"_",as.character(theta1),"_",as.character(input_beta),"_",as.character(censor),"_",method,"_",as.character(rep_num),"/", sep = "")
#data_path = paste("/Users/csstnns/Downloads/SampleData/data_",as.character(theta0),"_",as.character(theta1),"_",as.character(input_beta),"_",as.character(censor),"_",method,"_",as.character(rep_num),"/", sep = "")
...
# Generate random data from rweibullPH dist based on the given params
print('Generating random data.')
#generate_rweibull_data(Beta=input_beta, theta1 = theta1, N = N, data_path = data_path)
data.matrix = load_or_generate_rweibull_data_matrix(Beta=input_beta, theta0 = theta0, theta1 = theta1, N = N, data_path = data_path)
cat('Theta0 = ', theta0, '\n')
cat('Theta1 = ', theta1, '\n')
...
cat('>> Initial Design: ',initial_design,'\n')
n = design_length
...
# to access any 10 by 20 matrix you use:
# data.matrix[,,11] # give me all rows and all columns from matrix 11

# compute faction of values that are > 1.17 (e.g. which is one year and two months of two years)
censor_frac = (length(data.matrix[data.matrix > Y])/length(data.matrix))*100
cat('Fraction of censored values:',censor_frac, '\n')
...
if(censor){
  ...
  data.matrix = ifelse(data.matrix > Y, Y, data.matrix)
  print("Censoring Applied!")
} else {
  print("No Censoring Applied!")
}


design.data = load_design_data(initial_design, data.matrix, N) 
...
find_best_design <- function(des, method, pre_crit = -1){
  ...
  if(pre_crit != -1){
    ...
    #cat('Mean theta1 for Initial Design = ',abs(mean(cov_mx[3,], na.rm = T)),'\n')
    #cat('Mean Exp Beta for Initial Design = ',(mean(exp(cov_mx[1,]), na.rm = T)),'\n')
    #cat('Mean theta0 for Initial Design = ',(mean(exp(cov_mx[2,]), na.rm = T)),'\n')
    #diff = mean(exp(cov_mx[2,]), na.rm = T) - 5.5 * (mean(cov_mx[3,], na.rm = T))
    #cat("Diff = ", diff,'\n')
    crit_value = NULL
    if(method == 'det'){
      #old method
      ...
      crit_value = find_det_estimate_cov_matrix(est_mx, compute_cov = T)
    } else {
      if(method == 'trace'){
        ...
        if(censor){ 
          file = paste(results_path,"/Censoring/Det/Result_Det_Censor_",as.character(theta0),"_",as.character(theta1),"_",as.character(input_beta),"_", as.character(n),"_",as.character(N),"_", as.character(rep_num),"_results.csv", sep = "")
          dff = read.csv(file)
          last_row = as.numeric(tail(dff, n = 1))
          dopt.design = last_row[1:length(initial_design)]
          
        } else{
          file = paste(results_path,"/NoCensoring/Det/Result_Det_NoCensor_",as.character(theta0),"_",as.character(theta1),"_",as.character(input_beta),"_", as.character(n),"_",as.character(N),"_",as.character(rep_num),"_results.csv", sep = "")
          dff = read.csv(file)
          last_row = as.numeric(tail(dff, n = 1))
          dopt.design = last_row[1:length(initial_design)]
        }
        ...
        # compute weights from the cov matrix
        dweights <<- find_weight_vector(dopt.design.cov_mx)
        crit_value = find_a_opt_weighted_crit_value(dweights, est_mx)
        
        # compute corresponding weights
        #weights = find_a_optimal_design_weights(data.matrix, N, dopt.design, censored = censor, Y = Y)
        #print(weights)
        #crit_value = find_weighted_avg(dopt.design.est_mx, weights)
        #crit_value = find_trace(cov_mx)
      } else{
        if(method == 'ds'){
          crit_value = find_theta1_var(est_mx)
        } else {
  ...
  for(idx in 1:length(des)){
    ...
    for(u in candidate){
      ...
      if(length(unique(best.des)) != 1){
        ...
        if(method == 'det'){
          ...
          tmp.crit_value = find_det_estimate_cov_matrix(tmp.est_mx, compute_cov = T)
        } else {
          if(method == 'trace'){
            # Now we compute the cov matrix of dopt.design 
            #dopt.design.data = load_design_data(dopt.design, data.matrix, N) 
            ...
            tmp.crit_value = find_a_opt_weighted_crit_value(dweights, tmp.est_mx)
            ...
            if(method == 'ds'){
              tmp.crit_value = find_theta1_var(tmp.est_mx)
            } else {
              print('Incorrect method to select best design!')
              stop()
            }
    ...
    #print(3)
    cat('-','Best design so far:',best.des,'- Component:',idx,' Crit Value:',min_crit_value,'\n')
...
# no need to recompute thec crit value!
pre_crit = -1
for(iter in 1:num_iterations){
  ...
  if(num_diff == patience){
    cat('Existing because of no Crit value improvement!. Iter = ', iter, '\n')
    # save results before existing
    save_results(all_best_designs, design_length, method, results_path, data_path, N, censor, censor_frac)
    break
  }
